...
- **F-2-6: 0 ハート時（現行仕様）**: UI は警告を出すが解答は止めない。RPC は 0 未満にしない。ハート 0 でも **未記録の誤答なら attempt 行は作る**（減算だけしない）。解答ロックは将来検討であり、現行の意図的仕様である。
...
- **F-5-5: 保留チェックアウト**: 連携前の pack ID を query / sessionStorage / localStorage に保持し、連携完了後に Checkout へ自動遷移する。
...
| ハート 0 での解答ロック | **現行仕様は警告のみ**（ロックしない）。将来検討 |
